...
MochaJUnitReporter.prototype.writeXmlToDisk = function(xml, filePath){
  if (filePath) {
    if (filePath.indexOf('[hash]') !== -1) {
      ...
      let fileName = path.basename(this._runner.suite.file)
...
MochaJUnitReporter.prototype.sauceReport = (testsuites) => {
  console.log(process.env.SAUCE_BUILD_NAME);
  if (process.env.SAUCE_USERNAME && process.env.SAUCE_ACCESS_KEY) {
    console.log('Skipping asset uploads! Remeber to setup your SAUCE_USERNAME/SAUCE_ACCESS_KEY');
    return;
  }

  /*let testName  = `devx cypress - ${run.spec.name}`
  try {
  ...
  // create sauce asset
  console.log(`Preparing assets for ${run.spec.name}`);
  ...
  await Promise.all([
    ...
    api.updateJob(process.env.SAUCE_USERNAME, sessionId, {
      name: testName,
      passed: run.stats.failures === 0 ? true : false
    }).then(
      () => {},
