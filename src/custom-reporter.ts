...
function MochaJUnitReporter(runner: EventEmitter, options: object) {
  ...
  this._runner.on(
    ...
    function (test, err) {
      if (
        (test.err && test.err.expected !== undefined) ||
        (test.err && test.err.actual !== undefined)
      ) {
...
MochaJUnitReporter.prototype.report = function (
  testsuites: any[],
  sauceJson: any,
) {
 ...
 */
MochaJUnitReporter.prototype.getTestcaseData = function (
  test: any,
  err: any,
): object {
  const jenkinsMode = this._options.jenkinsMode;
  const flipClassAndName = this._options.testCaseSwitchClassnameAndName;
  const name = stripAnsi(
    jenkinsMode ? getJenkinsClassname(test, this._options) : test.fullTitle(),
  );
...
MochaJUnitReporter.prototype.removeInvalidCharacters = function (
  input: string,
): string {
  if (!input) {
    return input;
 ...
 */
MochaJUnitReporter.prototype.flush = function (
  testsuites: any[],
  specFile: string,
  sauceJson: any,
) {
 ...
 */
MochaJUnitReporter.prototype.getXml = function (
  testsuites: XmlSuite[],
): string {
  ...
  testsuites.forEach(function (suite) {
    ...
    _cases.forEach(function (testcase: XmlTestCaseContainer) {
      ...
      testcase.testcase[0]._attr.time = (
        testcase.testcase[0]._attr.time as number
      ).toFixed(4);
    });
 ...
 */
MochaJUnitReporter.prototype.writeSauceJsonToDisk = function (
  sauceJson: any,
  filePath: string,
  fileName: string,
): string {
  if (filePath) {
    ...
    try {
      fs.writeFileSync(
        filePath,
        JSON.stringify(sauceJson, undefined, 2),
        'utf-8',
      );
    } catch (exc) {
      console.error(exc);
 ...
 */
MochaJUnitReporter.prototype.writeXmlToDisk = function (
  xml: string,
  filePath: string,
  fileName: string,
): string {
